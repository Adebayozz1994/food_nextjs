...
export default function Products() {
  ...
  const fetchCartCount = async () => {
    try {
      ...
      const response = await axios.get<CartData>('http://localhost:5000/api/cart', {
  ...
  const handleAddToCart = async (productId: string) => {
    try {
      ...
      const cartResponse = await axios.get<CartData>('http://localhost:5000/api/cart', {
        headers: { Authorization: `Bearer ${token}` },
        withCredentials: true,
      });
                    ...
                    <div className="relative h-56">
                      ...
                      {!product.isAvailable && (
                        <div className="absolute inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center">
                          <span className="text-white text-lg font-semibold px-6 py-3 bg-black bg-opacity-50 rounded-lg">
                    ...
                    <div className="p-6">
                      ...
                      {product.isAvailable ? (
                        ...
                        <div className="space-y-3">
                          <button
                            disabled
                            className="w-full bg-gray-200 text-gray-500 px-6 py-3 rounded-lg cursor-not-allowed"
                          >
                            Out of Stock
                          </button>
